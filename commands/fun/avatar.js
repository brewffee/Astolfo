module.exports.run = async (message, args, flags) => {
  ...
  if (!args[0]) {
    ...
    return message.channel.send(
      errorEmbed.setDescription("Invalid syntax.\nUsage: `a!avatar [member]`")
    );
  } else if (
    args[0].startsWith("<@") &&
    args[0].endsWith(">") &&
    args[0].replace(/[^0-9]/g, "").length == 18
  ) {
  ...
  let foundMember;
  try {
    foundMember = await message.guild.members.fetch({
      user: memberID,
      force: true,
      cache: false,
    });
  ...
  message.channel.send(
    new (require("discord.js").MessageEmbed)()
      .setColor("#f7b2d9")
      .setImage(
        `${foundMember.user.displayAvatarURL({ size: 4096, dynamic: true })}`
      )
