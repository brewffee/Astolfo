module.exports.run = async (message, args) => {
    ...
    if (!isGlobal) {
        if (toBan == message.author.id) {
            return message.channel.send(errorEmbed.setDescription('You can\'t softban yourself!'));
        } else if (toBan == message.guild.me.id) {
            return message.channel.send(errorEmbed.setDescription('I can\'t softban myself!'));
        } else if (!message.guild.member(toBan).bannable || message.guild.member(toBan).permissions.has('ADMINISTRATOR')) {
            return message.channel.send(errorEmbed.setDescription('The specified member is immune to softbans!'));
        }
    }
    ...
    if (!isGlobal) {
        ...
        if (JSON.stringify(bans).includes(memberID)) {
            ...
            return message.channel.send(errorEmbed.setDescription('That user is already banned and cannot be softbanned!'));
        }
